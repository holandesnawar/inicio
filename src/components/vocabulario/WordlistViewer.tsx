    ...
    <div className="w-full max-w-sm mx-auto">
      ...
      <div className="bg-white rounded-2xl border border-[#DDE6F5] p-6 space-y-4">
        ...
        >
          ...
          <p className="text-[14px] text-[#1D0084] font-medium leading-snug">
            {word.example_nl}
          </p>
          {flipped && (
            <p className="text-[13px] text-[#5A6480] mt-1 leading-snug">
              {word.example_es}
            </p>
          )}
        </button>
        ...
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" d="M11 5l-7 7 7 7M18 5l-7 7 7 7" />
          </svg>
          Escuchar ejemplo
        </button>
      </div>
...
export default function WordlistViewer({ list }: { list: Woordenlijst }) {
  ...
  // Exercises are generated fresh each session (or on retry)
  const [exercises, setExercises] = useState<Exercise[]>(() =>
    generateExercises(list.words)
  );
  ...
  return (
    <>
      ...
      <section data-navbar="dark" className="relative bg-[#1D0084] overflow-hidden">
        ...
        <div className="relative max-w-2xl mx-auto px-6 pt-32 pb-12">
          <Link
            href="/vocabulario"
            className="inline-flex items-center gap-2 text-[13px] text-white/50 hover:text-white/80 transition-colors duration-200 mb-6"
          >
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-[14px] text-white/50">{list.subtitle} · {list.words.length} palabras · {list.level}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* ── Tab bar ── */}
      {phase !== "results" && (
        <div data-navbar="light" className="bg-white border-b border-[#DDE6F5] sticky top-[72px] z-40">
      ...
      {/* ── Content ── */}
      <section data-navbar="light" className="bg-white min-h-[70vh] py-10 pb-20">
        <div className="max-w-2xl mx-auto px-6">
          ...
          {phase === "learn" && (
            <div className="space-y-8">
              ...
              {/* Card with key to reset internal state on word change */}
...
/* ─────────────────────────────────────────────────────────────────────────────
   PRACTICE PHASE (extracted to own component to manage answered state cleanly)
───────────────────────────────────────────────────────────────────────────── */
