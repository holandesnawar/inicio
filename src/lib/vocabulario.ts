...
export interface Word {
  ...
  spanish:    string
  article:    Article
  emoji:      string    // visual fallback (always available)
  color:      string    // hex bg for emoji card
  image?:     string    // optional photo URL (future)
  audio?:     string    // optional audio URL (fallback: TTS)
  example_nl: string
  example_es: string
  category:   string
  difficulty: 'A0' | 'A1'
...
export type ExerciseType =
  | 'word_to_translation'   // See Dutch word → pick Spanish
  | 'translation_to_word'   // See Spanish → pick Dutch
...
/* ── Future user-progress types (ready to wire to localStorage / API) ── */
...
/**
 * Generate a randomised exercise set from a word list.
 * Each word gets at least one exercise; ~40% get a second reinforcement exercise.
 * The architecture is designed so you can later weight exercises by WordProgress.
 */
export function generateExercises(words: Word[]): Exercise[] {
  ...
  for (const word of shuffled) {
    ...
    // Alternate types to keep practice varied
